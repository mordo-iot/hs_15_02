...
<html>
  ...
  <script type="text/javascript">
	...
	function sure4del() {
		...
		$.ajax({
			type:"post",
			url:"currentAlarm.do?cancelAlarm",
			timeout: 3000,
			dataType:"text",
			async:false,
	...
	function getCurrentAlarmInfo() {
		...
		$.ajax({
			...
			success:function(data){
				...
				if (alarmhash != d.hashCode) {  //报警信息发生变化时，刷新页面内容
        ...
        <div id="Caution_1">
          <ul class="warningMessage">
            <li style="background-color:#2e94e3;color:#fff;font-weight:bold;">
              <span>报警时间</span>
              <span>
                <select class="select-style" id="alarmTypeSelector" onchange="getCurrentAlarmInfo()" style="background: #2E94E3 url('./images/tri.png') no-repeat scroll 108px 0px;">
