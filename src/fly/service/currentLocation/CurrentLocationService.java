...
public class CurrentLocationService {
	   ...
	   /**
		...
		public List<Object> getListByCondition(Map<String,Object> queryMap) {
			...
			QueryCondition qc = new QueryCondition(CurrentLocationEntity.ID, QueryCondition.gt, "0");				
				    ...
				    if(leaved_in!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.LEAVED, QueryCondition.in, leaved_in));}
				  															  					if(leavedupdatetime_gt!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.LEAVEDUPDATETIME, QueryCondition.gt, leavedupdatetime_gt));}
					if(leavedupdatetime_ge!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.LEAVEDUPDATETIME, QueryCondition.ge, leavedupdatetime_ge));}
					if(leavedupdatetime_lt!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.LEAVEDUPDATETIME, QueryCondition.lt, leavedupdatetime_lt));}
					if(leavedupdatetime_le!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.LEAVEDUPDATETIME, QueryCondition.le, leavedupdatetime_le));}
				    ...
				    if(manualalarm_in!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.MANUALALARM, QueryCondition.in, manualalarm_in));}
				  															  					if(bodyupdatetime_gt!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.BODYUPDATETIME, QueryCondition.gt, bodyupdatetime_gt));}
					if(bodyupdatetime_ge!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.BODYUPDATETIME, QueryCondition.ge, bodyupdatetime_ge));}
					if(bodyupdatetime_lt!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.BODYUPDATETIME, QueryCondition.lt, bodyupdatetime_lt));}
					if(bodyupdatetime_le!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.BODYUPDATETIME, QueryCondition.le, bodyupdatetime_le));}
				    ...
				    if(moving_in!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.MOVING, QueryCondition.in, moving_in));}
				  															  					if(movingupdatetime_gt!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.MOVINGUPDATETIME, QueryCondition.gt, movingupdatetime_gt));}
					if(movingupdatetime_ge!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.MOVINGUPDATETIME, QueryCondition.ge, movingupdatetime_ge));}
					if(movingupdatetime_lt!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.MOVINGUPDATETIME, QueryCondition.lt, movingupdatetime_lt));}
					if(movingupdatetime_le!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.MOVINGUPDATETIME, QueryCondition.le, movingupdatetime_le));}
				    ...
				    if(power_in!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.POWER, QueryCondition.in, power_in));}
				  															  					if(devupdatetime_gt!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.DEVUPDATETIME, QueryCondition.gt, devupdatetime_gt));}
					if(devupdatetime_ge!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.DEVUPDATETIME, QueryCondition.ge, devupdatetime_ge));}
					if(devupdatetime_lt!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.DEVUPDATETIME, QueryCondition.lt, devupdatetime_lt));}
					if(devupdatetime_le!=null){qc.andCondition(new QueryCondition(CurrentLocationEntity.DEVUPDATETIME, QueryCondition.le, devupdatetime_le));}
		...
		public PageList getListByCondition(Map<String,Object> queryMap,List<OrderVO> orderList,int pageno,int pagesize,Boolean devShow,Boolean positionShow) {
            ...
            pagelist = dbManager.queryByConditions(CurrentLocationEntity.class,qc,oc,pageno,pagesize);	           
                                    int a=0;
            					             	                 				     				     if(devShow!=null&&devShow.booleanValue()){
						a++;
					 }
				...
				for(int i=0;i<pagelist.getResultList().size();i++){
					CurrentLocationEntity obj = (CurrentLocationEntity)pagelist.getResultList().get(i);
										 	                 				     				        //查询关联内容
						if(devShow!=null&&devShow.booleanValue()&&obj!=null&&obj.getDevId()>0){
							DevEntity dev = (DevEntity)dbManager.getById(obj.getDevId(), DevEntity.class);
							obj.setDev(dev);        	  	  
