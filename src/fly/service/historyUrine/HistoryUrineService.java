...
public class HistoryUrineService {
	   ...
	   /**
		...
		public List<Object> getListByCondition(Map<String,Object> queryMap) {
			...
			QueryCondition qc = new QueryCondition(HistoryUrineEntity.ID, QueryCondition.gt, "0");				
				    ...
				    if(alarm_in!=null){qc.andCondition(new QueryCondition(HistoryUrineEntity.ALARM, QueryCondition.in, alarm_in));}
				  															  					if(alarmupdatetime_gt!=null){qc.andCondition(new QueryCondition(HistoryUrineEntity.ALARMUPDATETIME, QueryCondition.gt, alarmupdatetime_gt));}
					if(alarmupdatetime_ge!=null){qc.andCondition(new QueryCondition(HistoryUrineEntity.ALARMUPDATETIME, QueryCondition.ge, alarmupdatetime_ge));}
					if(alarmupdatetime_lt!=null){qc.andCondition(new QueryCondition(HistoryUrineEntity.ALARMUPDATETIME, QueryCondition.lt, alarmupdatetime_lt));}
					if(alarmupdatetime_le!=null){qc.andCondition(new QueryCondition(HistoryUrineEntity.ALARMUPDATETIME, QueryCondition.le, alarmupdatetime_le));}
		...
		public PageList getListByCondition(Map<String,Object> queryMap,List<OrderVO> orderList,int pageno,int pagesize,Boolean devShow) {
            ...
            pagelist = dbManager.queryByConditions(HistoryUrineEntity.class,qc,oc,pageno,pagesize);	           
                                    int a=0;
            					             	                 				     				     if(devShow!=null&&devShow.booleanValue()){
						a++;
					 }
				...
				for(int i=0;i<pagelist.getResultList().size();i++){
					HistoryUrineEntity obj = (HistoryUrineEntity)pagelist.getResultList().get(i);
										 	                 				     				        //查询关联内容
						if(devShow!=null&&devShow.booleanValue()&&obj!=null&&obj.getDevId()>0){
							DevEntity dev = (DevEntity)dbManager.getById(obj.getDevId(), DevEntity.class);
							obj.setDev(dev);        	  	  
