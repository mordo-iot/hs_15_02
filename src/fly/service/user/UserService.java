...
public class UserService {
				...
				for (UserEntity user : userList) {
					if (user != null) {
						if (user.getUserLogList() != null
								&& user.getUserLogList().size() > 0) {
							// 关联信息保存
							for (UserLogEntity userLogEntity : user
									.getUserLogList()) {
								dbManager.saveNoTransaction(userLogEntity);
							}
						}
	 ...
	 */
	public List<Object> getListByCondition(Map<String, Object> queryMap,
			List<OrderVO> orderList, Boolean userLogListShow) {
		List<Object> list = null;
		if (queryMap == null) {
			queryMap = new HashMap<String, Object>();
		}
	...
	public PageList getListByCondition(Map<String, Object> queryMap,
		...
		pagelist = dbManager.queryByConditions(UserEntity.class, qc, oc,
				pageno, pagesize);
		int a = 0;
		if (userLogListShow) {
			a++;
