...
public class UserLogService {
	 ...
	 */
	public List<Object> getListByCondition(Map<String, Object> queryMap,
			List<OrderVO> orderList, Boolean userShow) {
		List<Object> list = null;
		if (queryMap == null) {
			queryMap = new HashMap<String, Object>();
		}
	 ...
	 */
	public PageList getListByCondition(Map<String, Object> queryMap,
			List<OrderVO> orderList, int pageno, int pagesize, Boolean userShow) {
		PageList pagelist = null;
		if (queryMap == null) {
			queryMap = new HashMap<String, Object>();
		}
		...
		pagelist = dbManager.queryByConditions(UserLogEntity.class, qc, oc,
				pageno, pagesize);
		int a = 0;
		if (userShow) {
			a++;
		...
		if (a > 0 && pagelist != null && pagelist.getResultList() != null
			...
			for (int i = 0; i < pagelist.getResultList().size(); i++) {
				UserLogEntity obj = (UserLogEntity) pagelist.getResultList()
						.get(i);
				// 查询关联内容
				if (userShow != null && userShow.booleanValue() && obj != null
						&& obj.getUserId() > 0) {
