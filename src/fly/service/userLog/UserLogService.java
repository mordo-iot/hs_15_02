		...
		public List<Object> getListByCondition(Map<String,Object> queryMap) {
			...
			QueryCondition qc = new QueryCondition(UserLogEntity.ID, QueryCondition.gt, "0");				
					...
					if(level_in!=null){qc.andCondition(new QueryCondition(UserLogEntity.LEVEL, QueryCondition.in, level_in));}
																if(createdate!=null){qc.andCondition(new QueryCondition(UserLogEntity.CREATEDATE, QueryCondition.eq, createdate));}
		            if(createdate_like!=null){qc.andCondition(new QueryCondition(UserLogEntity.CREATEDATE, QueryCondition.like, createdate_like));}
		            if(createdate_isNull!=null){qc.andCondition(new QueryCondition(UserLogEntity.CREATEDATE, QueryCondition.isNull, createdate_isNull));}
		            if(createdate_isNotNull!=null){qc.andCondition(new QueryCondition(UserLogEntity.CREATEDATE, QueryCondition.isNotNull, createdate_isNotNull));}
		            if(createdate_in!=null){qc.andCondition(new QueryCondition(UserLogEntity.CREATEDATE, QueryCondition.in, createdate_in));}
