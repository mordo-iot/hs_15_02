		...
		public List<Object> getListByCondition(Map<String,Object> queryMap) {
			...
			QueryCondition qc = new QueryCondition(PositionEntity.ID, QueryCondition.gt, "0");				
					...
					if(parentId_in!=null){qc.andCondition(new QueryCondition(PositionEntity.PARENT_ID, QueryCondition.in, parentId_in));}
																if(createdate!=null){qc.andCondition(new QueryCondition(PositionEntity.CREATEDATE, QueryCondition.eq, createdate));}
		            if(createdate_like!=null){qc.andCondition(new QueryCondition(PositionEntity.CREATEDATE, QueryCondition.like, createdate_like));}
		            if(createdate_isNull!=null){qc.andCondition(new QueryCondition(PositionEntity.CREATEDATE, QueryCondition.isNull, createdate_isNull));}
		            if(createdate_isNotNull!=null){qc.andCondition(new QueryCondition(PositionEntity.CREATEDATE, QueryCondition.isNotNull, createdate_isNotNull));}
		            if(createdate_in!=null){qc.andCondition(new QueryCondition(PositionEntity.CREATEDATE, QueryCondition.in, createdate_in));}
																if(updatedate!=null){qc.andCondition(new QueryCondition(PositionEntity.UPDATEDATE, QueryCondition.eq, updatedate));}
		            if(updatedate_like!=null){qc.andCondition(new QueryCondition(PositionEntity.UPDATEDATE, QueryCondition.like, updatedate_like));}
		            if(updatedate_isNull!=null){qc.andCondition(new QueryCondition(PositionEntity.UPDATEDATE, QueryCondition.isNull, updatedate_isNull));}
		            if(updatedate_isNotNull!=null){qc.andCondition(new QueryCondition(PositionEntity.UPDATEDATE, QueryCondition.isNotNull, updatedate_isNotNull));}
		            if(updatedate_in!=null){qc.andCondition(new QueryCondition(PositionEntity.UPDATEDATE, QueryCondition.in, updatedate_in));}
