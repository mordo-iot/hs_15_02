		...
		public List<Object> getListByCondition(Map<String,Object> queryMap) {
			...
			QueryCondition qc = new QueryCondition(HistoryKeyalarmEntity.ID, QueryCondition.gt, "0");				
		            ...
		            if(alarm_in!=null){qc.andCondition(new QueryCondition(HistoryKeyalarmEntity.ALARM, QueryCondition.in, alarm_in));}
																if(alarmupdatetime!=null){qc.andCondition(new QueryCondition(HistoryKeyalarmEntity.ALARMUPDATETIME, QueryCondition.eq, alarmupdatetime));}
		            if(alarmupdatetime_like!=null){qc.andCondition(new QueryCondition(HistoryKeyalarmEntity.ALARMUPDATETIME, QueryCondition.like, alarmupdatetime_like));}
		            if(alarmupdatetime_isNull!=null){qc.andCondition(new QueryCondition(HistoryKeyalarmEntity.ALARMUPDATETIME, QueryCondition.isNull, alarmupdatetime_isNull));}
		            if(alarmupdatetime_isNotNull!=null){qc.andCondition(new QueryCondition(HistoryKeyalarmEntity.ALARMUPDATETIME, QueryCondition.isNotNull, alarmupdatetime_isNotNull));}
		            if(alarmupdatetime_in!=null){qc.andCondition(new QueryCondition(HistoryKeyalarmEntity.ALARMUPDATETIME, QueryCondition.in, alarmupdatetime_in));}
