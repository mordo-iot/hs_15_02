...
public class HistoryLocationPosService {
	...
	public boolean save(HistoryLocationPosEntity historyLocationPos) {
		...
		if (historyLocationPos != null) {
			...
			try {
				tx.beginTransaction();
				dbManager.saveNoTransaction(historyLocationPos);
				// 关联信息保存
				if (historyLocationPos.getDev() != null) {
					dbManager.saveNoTransaction(historyLocationPos.getDev());
				}
				tx.commitAndClose();
			} catch (Exception e) {
	...
	public List<Object> getListByCondition(Map<String, Object> queryMap) {
		...
		if (leavedupdatetime != null) {
			qc.andCondition(new QueryCondition(
					...
					QueryCondition.eq, leavedupdatetime));
		}
		...
		if (id != null && id > 0) {
			...
			try {
				...
				if (delDev != null && delDev.booleanValue()) {
					...
					if (historyLocationPos.getDevId() != null) {
