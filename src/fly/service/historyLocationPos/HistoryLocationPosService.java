...
public class HistoryLocationPosService {
	private static Logger logger = Logger
			.getLogger(HistoryLocationPosService.class);
			...
			try {
				...
				for (HistoryLocationPosEntity historyLocationPos : historyLocationPosList) {
					if (historyLocationPos != null) {
						...
						// 关联信息保存
						PositionEntity position = historyLocationPos
								.getPosition();
						if (position != null) {
							dbManager.saveNoTransaction(position);
							historyLocationPos.setPositionId(position.getId());
						...
						result = dbManager
								.saveNoTransaction(historyLocationPos);
					}
				}
				tx.commitAndClose();
			} catch (Exception e) {
	...
	public HistoryLocationPosEntity getById(Integer id, Boolean devShow,
		...
		if (id != null) {
			obj = (HistoryLocationPosEntity) dbManager.getById(id,
	...
	public List<Object> getListByCondition(Map<String, Object> queryMap) {
		...
		QueryCondition qc = new QueryCondition(HistoryLocationPosEntity.ID,
				QueryCondition.gt, "0");
		if (id != null) {
			qc.andCondition(new QueryCondition(HistoryLocationPosEntity.ID,
		...
		if (devId != null) {
			qc.andCondition(new QueryCondition(HistoryLocationPosEntity.DEV_ID,
		...
		if (positionId != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.POSITION_ID, QueryCondition.eq,
					positionId));
		}
		if (positionId_gt != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.POSITION_ID, QueryCondition.gt,
					positionId_gt));
		}
		if (positionId_ge != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.POSITION_ID, QueryCondition.ge,
					positionId_ge));
		}
		if (positionId_lt != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.POSITION_ID, QueryCondition.lt,
					positionId_lt));
		}
		if (positionId_le != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.POSITION_ID, QueryCondition.le,
					positionId_le));
		}
		if (positionId_in != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.POSITION_ID, QueryCondition.in,
					positionId_in));
		}
		if (currlog != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLOG, QueryCondition.eq,
					currlog));
		}
		if (currlog_gt != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLOG, QueryCondition.gt,
					currlog_gt));
		}
		if (currlog_ge != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLOG, QueryCondition.ge,
					currlog_ge));
		}
		if (currlog_lt != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLOG, QueryCondition.lt,
					currlog_lt));
		}
		if (currlog_le != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLOG, QueryCondition.le,
					currlog_le));
		}
		if (currlog_in != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLOG, QueryCondition.in,
					currlog_in));
		}
		if (currlat != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLAT, QueryCondition.eq,
					currlat));
		}
		if (currlat_gt != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLAT, QueryCondition.gt,
					currlat_gt));
		}
		if (currlat_ge != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLAT, QueryCondition.ge,
					currlat_ge));
		}
		if (currlat_lt != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLAT, QueryCondition.lt,
					currlat_lt));
		}
		if (currlat_le != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLAT, QueryCondition.le,
					currlat_le));
		}
		if (currlat_in != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.CURRLAT, QueryCondition.in,
					currlat_in));
		}
		if (leaved != null) {
		...
		if (leavedupdatetime_gt != null) {
			qc.andCondition(new QueryCondition(
					...
					QueryCondition.gt, leavedupdatetime_gt));
		}
		if (leavedupdatetime_ge != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.LEAVEDUPDATETIME,
					QueryCondition.ge, leavedupdatetime_ge));
		}
		if (leavedupdatetime_lt != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.LEAVEDUPDATETIME,
					QueryCondition.lt, leavedupdatetime_lt));
		}
		if (leavedupdatetime_le != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationPosEntity.LEAVEDUPDATETIME,
					QueryCondition.le, leavedupdatetime_le));
		}
	...
	public List<Object> getListByCondition(Map<String, Object> queryMap,
		...
		list = dbManager.queryByConditions(HistoryLocationPosEntity.class, qc,
				oc);
		int a = 0;
		if (devShow) {
			a++;
		...
		if (a > 0 && list != null && list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				HistoryLocationPosEntity obj = (HistoryLocationPosEntity) list
						.get(i);
				// 查询关联内容
				if (devShow != null && devShow.booleanValue() && obj != null
						&& obj.getDevId() > 0) {
	...
	public PageList getListByCondition(Map<String, Object> queryMap,
		...
		pagelist = dbManager.queryByConditions(HistoryLocationPosEntity.class,
		...
		if (a > 0 && pagelist != null && pagelist.getResultList() != null
			...
			for (int i = 0; i < pagelist.getResultList().size(); i++) {
				HistoryLocationPosEntity obj = (HistoryLocationPosEntity) pagelist
						.getResultList().get(i);
				// 查询关联内容
				if (devShow != null && devShow.booleanValue() && obj != null
						&& obj.getDevId() > 0) {
		...
		if (id != null && id > 0) {
			...
			try {
				...
				if (delDev != null && delDev.booleanValue()) {
					HistoryLocationPosEntity historyLocationPos = (HistoryLocationPosEntity) dbManager
							.getById(id, HistoryLocationPosEntity.class);
					if (historyLocationPos != null
							&& historyLocationPos.getDevId() != null) {
						dbManager.delNoTransaction(
								historyLocationPos.getDevId(), DevEntity.class);
					}
				}
				// 删除关联信息
				if (delPosition != null && delPosition.booleanValue()) {
					HistoryLocationPosEntity historyLocationPos = (HistoryLocationPosEntity) dbManager
							.getById(id, HistoryLocationPosEntity.class);
					if (historyLocationPos != null
							&& historyLocationPos.getPositionId() != null) {
						dbManager.delNoTransaction(
								historyLocationPos.getPositionId(),
