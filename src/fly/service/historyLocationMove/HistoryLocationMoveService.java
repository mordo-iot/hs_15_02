...
public class HistoryLocationMoveService {
	...
	public List<Object> getListByCondition(Map<String, Object> queryMap) {
		...
		if (devId != null) {
			qc.andCondition(new QueryCondition(
					HistoryLocationMoveEntity.DEV_ID, QueryCondition.eq, devId));
		}
		if (devId_gt != null) {
			qc.andCondition(new QueryCondition(
	...
	public List<Object> getListByCondition(Map<String, Object> queryMap,
		...
		list = dbManager.queryByConditions(HistoryLocationMoveEntity.class, qc,
				oc);
		int a = 0;
		if (devShow) {
			a++;
		...
		if (a > 0 && list != null && list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				HistoryLocationMoveEntity obj = (HistoryLocationMoveEntity) list
						.get(i);
				// 查询关联内容
				if (devShow != null && devShow.booleanValue() && obj != null
						&& obj.getDevId() > 0) {
	...
	public PageList getListByCondition(Map<String, Object> queryMap,
		...
		pagelist = dbManager.queryByConditions(HistoryLocationMoveEntity.class,
		...
		if (a > 0 && pagelist != null && pagelist.getResultList() != null
			...
			for (int i = 0; i < pagelist.getResultList().size(); i++) {
				HistoryLocationMoveEntity obj = (HistoryLocationMoveEntity) pagelist
						.getResultList().get(i);
				// 查询关联内容
				if (devShow != null && devShow.booleanValue() && obj != null
						&& obj.getDevId() > 0) {
		...
		if (id != null && id > 0) {
			...
			try {
				...
				if (delDev != null && delDev.booleanValue()) {
					...
					if (historyLocationMove != null
							&& historyLocationMove.getDevId() != null) {
						dbManager
								.delNoTransaction(
