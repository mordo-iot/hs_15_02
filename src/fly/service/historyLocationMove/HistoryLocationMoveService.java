		...
		public List<Object> getListByCondition(Map<String,Object> queryMap) {
			...
			QueryCondition qc = new QueryCondition(HistoryLocationMoveEntity.ID, QueryCondition.gt, "0");				
		            ...
		            if(moving_in!=null){qc.andCondition(new QueryCondition(HistoryLocationMoveEntity.MOVING, QueryCondition.in, moving_in));}
																if(movingupdatetime!=null){qc.andCondition(new QueryCondition(HistoryLocationMoveEntity.MOVINGUPDATETIME, QueryCondition.eq, movingupdatetime));}
		            if(movingupdatetime_like!=null){qc.andCondition(new QueryCondition(HistoryLocationMoveEntity.MOVINGUPDATETIME, QueryCondition.like, movingupdatetime_like));}
		            if(movingupdatetime_isNull!=null){qc.andCondition(new QueryCondition(HistoryLocationMoveEntity.MOVINGUPDATETIME, QueryCondition.isNull, movingupdatetime_isNull));}
		            if(movingupdatetime_isNotNull!=null){qc.andCondition(new QueryCondition(HistoryLocationMoveEntity.MOVINGUPDATETIME, QueryCondition.isNotNull, movingupdatetime_isNotNull));}
		            if(movingupdatetime_in!=null){qc.andCondition(new QueryCondition(HistoryLocationMoveEntity.MOVINGUPDATETIME, QueryCondition.in, movingupdatetime_in));}
