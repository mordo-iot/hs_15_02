...
public class HistoryWandaiService {
	...
	public List<Object> getListByCondition(Map<String, Object> queryMap) {
		...
		if (alarmupdatetime_gt != null) {
			qc.andCondition(new QueryCondition(
					HistoryWandaiEntity.ALARMUPDATETIME, QueryCondition.gt,
					alarmupdatetime_gt));
		}
		if (alarmupdatetime_ge != null) {
			qc.andCondition(new QueryCondition(
					HistoryWandaiEntity.ALARMUPDATETIME, QueryCondition.ge,
					alarmupdatetime_ge));
		}
		if (alarmupdatetime_lt != null) {
			qc.andCondition(new QueryCondition(
					HistoryWandaiEntity.ALARMUPDATETIME, QueryCondition.lt,
					alarmupdatetime_lt));
		}
		if (alarmupdatetime_le != null) {
			qc.andCondition(new QueryCondition(
					HistoryWandaiEntity.ALARMUPDATETIME, QueryCondition.le,
					alarmupdatetime_le));
		}
	...
	public PageList getListByCondition(Map<String, Object> queryMap,
		...
		pagelist = dbManager.queryByConditions(HistoryWandaiEntity.class, qc,
				oc, pageno, pagesize);
		int a = 0;
		if (devShow) {
			a++;
		...
		if (a > 0 && pagelist != null && pagelist.getResultList() != null
			...
			for (int i = 0; i < pagelist.getResultList().size(); i++) {
				HistoryWandaiEntity obj = (HistoryWandaiEntity) pagelist
						.getResultList().get(i);
				// 查询关联内容
				if (devShow != null && devShow.booleanValue() && obj != null
						&& obj.getDevId() > 0) {
