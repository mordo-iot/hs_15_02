...
public class HistoryWandaiService {
	...
	public boolean save(HistoryWandaiEntity historyWandai) {
		...
		if (historyWandai != null) {
			...
			try {
				tx.beginTransaction();
				dbManager.saveNoTransaction(historyWandai);
				// 关联信息保存
				if (historyWandai.getDev() != null) {
					dbManager.saveNoTransaction(historyWandai.getDev());
				}
				tx.commitAndClose();
			} catch (Exception e) {
	...
	public HistoryWandaiEntity getById(Integer id, Boolean devShow) {
		...
		if (id != null) {
			obj = (HistoryWandaiEntity) dbManager.getById(id,
					HistoryWandaiEntity.class);
			// 查询关联内容
	...
	public List<Object> getListByCondition(Map<String, Object> queryMap) {
		...
		QueryCondition qc = new QueryCondition(HistoryWandaiEntity.ID,
				QueryCondition.gt, "0");
		if (id != null) {
			qc.andCondition(new QueryCondition(HistoryWandaiEntity.ID,
		...
		if (devId != null) {
			qc.andCondition(new QueryCondition(HistoryWandaiEntity.DEV_ID,
		...
		if (alarm != null) {
			qc.andCondition(new QueryCondition(HistoryWandaiEntity.ALARM,
		...
		if (alarmupdatetime != null) {
			qc.andCondition(new QueryCondition(
					HistoryWandaiEntity.ALARMUPDATETIME, QueryCondition.eq,
					alarmupdatetime));
		}
		if (alarmupdatetime_like != null) {
			qc.andCondition(new QueryCondition(
					HistoryWandaiEntity.ALARMUPDATETIME, QueryCondition.like,
					alarmupdatetime_like));
		}
		if (alarmupdatetime_isNull != null) {
			qc.andCondition(new QueryCondition(
					HistoryWandaiEntity.ALARMUPDATETIME, QueryCondition.isNull,
					alarmupdatetime_isNull));
		}
		if (alarmupdatetime_isNotNull != null) {
			qc.andCondition(new QueryCondition(
					HistoryWandaiEntity.ALARMUPDATETIME,
					QueryCondition.isNotNull, alarmupdatetime_isNotNull));
		}
		if (alarmupdatetime_in != null) {
			qc.andCondition(new QueryCondition(
					HistoryWandaiEntity.ALARMUPDATETIME, QueryCondition.in,
					alarmupdatetime_in));
		}
	...
	public PageList getListByCondition(Map<String, Object> queryMap,
		...
		pagelist = dbManager.queryByCondition(HistoryWandaiEntity.class, qc,
				pageno, pagesize);
		return pagelist;
	}
		...
		if (id != null && id > 0) {
			...
			try {
				...
				if (delDev != null && delDev.booleanValue()) {
					HistoryWandaiEntity historyWandai = (HistoryWandaiEntity) dbManager
							.getById(id, HistoryWandaiEntity.class);
					if (historyWandai.getDevId() != null) {
						dbManager.delNoTransaction(historyWandai.getDevId(),
								DevEntity.class);
					}
				}
				tx.commitAndClose();
