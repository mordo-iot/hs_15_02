...
public class AlarmHistoryService {
	...
	public boolean save(AlarmHistoryEntity alarmHistory) {
		...
		if (alarmHistory != null) {
			...
			try {
				...
				// 关联信息保存
				if (alarmHistory.getDev() != null) {
					dbManager.saveNoTransaction(alarmHistory.getDev());
				}
				tx.commitAndClose();
			} catch (Exception e) {
	...
	public AlarmHistoryEntity getById(Integer id, Boolean devShow) {
		...
		if (id != null) {
			obj = (AlarmHistoryEntity) dbManager.getById(id,
	...
	public List<Object> getListByCondition(Map<String, Object> queryMap) {
		...
		QueryCondition qc = new QueryCondition(AlarmHistoryEntity.ID,
				QueryCondition.gt, "0");
		if (id != null) {
			qc.andCondition(new QueryCondition(AlarmHistoryEntity.ID,
		...
		if (code != null) {
			qc.andCondition(new QueryCondition(AlarmHistoryEntity.CODE,
		...
		if (devId != null) {
			qc.andCondition(new QueryCondition(AlarmHistoryEntity.DEV_ID,
		...
		if (id != null && id > 0) {
			...
			try {
				...
				if (delDev != null && delDev.booleanValue()) {
					AlarmHistoryEntity alarmHistory = (AlarmHistoryEntity) dbManager
							.getById(id, AlarmHistoryEntity.class);
					if (alarmHistory.getDevId() != null) {
						dbManager.delNoTransaction(alarmHistory.getDevId(),
								DevEntity.class);
					}
				}
				tx.commitAndClose();
