...
public class CurrentTizhengBedService {
	...
	public boolean save(CurrentTizhengBedEntity currentTizhengBed) {
		...
		if (currentTizhengBed != null) {
			...
			try {
				tx.beginTransaction();
				dbManager.saveNoTransaction(currentTizhengBed);
				// 关联信息保存
				if (currentTizhengBed.getDev() != null) {
					dbManager.saveNoTransaction(currentTizhengBed.getDev());
				}
				tx.commitAndClose();
			} catch (Exception e) {
	...
	public List<Object> getListByCondition(Map<String, Object> queryMap) {
		...
		if (alarmupdatetime != null) {
			qc.andCondition(new QueryCondition(
					CurrentTizhengBedEntity.ALARMUPDATETIME, QueryCondition.eq,
					alarmupdatetime));
		}
		if (alarmupdatetime_like != null) {
			qc.andCondition(new QueryCondition(
					CurrentTizhengBedEntity.ALARMUPDATETIME,
		...
		if (alarmupdatetime_in != null) {
			qc.andCondition(new QueryCondition(
					CurrentTizhengBedEntity.ALARMUPDATETIME, QueryCondition.in,
					alarmupdatetime_in));
		}
		if (heart != null) {
		...
		if (devupdatetime != null) {
			qc.andCondition(new QueryCondition(
					CurrentTizhengBedEntity.DEVUPDATETIME, QueryCondition.eq,
					devupdatetime));
		}
		if (devupdatetime_like != null) {
			qc.andCondition(new QueryCondition(
					CurrentTizhengBedEntity.DEVUPDATETIME, QueryCondition.like,
					devupdatetime_like));
		}
		if (devupdatetime_isNull != null) {
			qc.andCondition(new QueryCondition(
					CurrentTizhengBedEntity.DEVUPDATETIME,
		...
		if (devupdatetime_in != null) {
			qc.andCondition(new QueryCondition(
					CurrentTizhengBedEntity.DEVUPDATETIME, QueryCondition.in,
					devupdatetime_in));
		}
		...
		if (id != null && id > 0) {
			...
			try {
				...
				if (delDev != null && delDev.booleanValue()) {
					...
					if (currentTizhengBed.getDevId() != null) {
