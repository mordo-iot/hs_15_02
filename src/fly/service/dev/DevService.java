		...
		if (dev != null) {
			...
			try {
				...
				if (dev.getAlarmCurrentList() != null
					...
					// 关联信息保存
					for (AlarmCurrentEntity alarmCurrent : dev
							.getAlarmCurrentList()) {
				...
				if (dev.getAlarmHistoryList() != null
					...
					// 关联信息保存
					for (AlarmHistoryEntity alarmHistory : dev
							.getAlarmHistoryList()) {
						dbManager.saveNoTransaction(alarmHistory);
					}
				}
				if (dev.getCurrentBedList() != null
						&& dev.getCurrentBedList().size() > 0) {
					// 关联信息保存
					for (CurrentBedEntity currentBed : dev.getCurrentBedList()) {
				...
				if (dev.getCurrentDoorList() != null
					...
					// 关联信息保存
					for (CurrentDoorEntity currentDoor : dev
							.getCurrentDoorList()) {
				...
				if (dev.getCurrentGatewayList() != null
					...
					// 关联信息保存
					for (CurrentGatewayEntity currentGateway : dev
				...
				if (dev.getCurrentKeyalarmList() != null
					...
					// 关联信息保存
					for (CurrentKeyalarmEntity currentKeyalarm : dev
				...
				if (dev.getCurrentLocationList() != null
					...
					// 关联信息保存
					for (CurrentLocationEntity currentLocation : dev
				...
				if (dev.getCurrentUrineList() != null
					...
					// 关联信息保存
					for (CurrentUrineEntity currentUrine : dev
							.getCurrentUrineList()) {
				...
				if (dev.getCurrentWandaiList() != null
					...
					// 关联信息保存
					for (CurrentWandaiEntity currentWandai : dev
				...
				if (dev.getHistoryBedList() != null
					...
					// 关联信息保存
					for (HistoryBedEntity historyBed : dev.getHistoryBedList()) {
				...
				if (dev.getHistoryDoorList() != null
					...
					// 关联信息保存
					for (HistoryDoorEntity historyDoor : dev
							.getHistoryDoorList()) {
				...
				if (dev.getHistoryKeyalarmList() != null
					...
					// 关联信息保存
					for (HistoryKeyalarmEntity historyKeyalarm : dev
				...
				if (dev.getHistoryLocationBodyList() != null
					...
					// 关联信息保存
					for (HistoryLocationBodyEntity historyLocationBody : dev
							.getHistoryLocationBodyList()) {
						dbManager.saveNoTransaction(historyLocationBody);
					}
				}
				if (dev.getHistoryLocationManualList() != null
						&& dev.getHistoryLocationManualList().size() > 0) {
					// 关联信息保存
					for (HistoryLocationManualEntity historyLocationManual : dev
				...
				if (dev.getHistoryLocationMoveList() != null
					...
					// 关联信息保存
					for (HistoryLocationMoveEntity historyLocationMove : dev
				...
				if (dev.getHistoryLocationPosList() != null
					...
					// 关联信息保存
					for (HistoryLocationPosEntity historyLocationPos : dev
				...
				if (dev.getHistoryUrineList() != null
					...
					// 关联信息保存
					for (HistoryUrineEntity historyUrine : dev
							.getHistoryUrineList()) {
						dbManager.saveNoTransaction(historyUrine);
					}
				}
				tx.commitAndClose();
			} catch (Exception e) {
				...
				for (DevEntity dev : devList) {
					if (dev != null) {
						...
						if (dev.getCurrentBedList() != null
								&& dev.getCurrentBedList().size() > 0) {
							// 关联信息保存
							for (CurrentBedEntity currentBed : dev
									.getCurrentBedList()) {
								dbManager.saveNoTransaction(currentBed);
							}
						}
						...
						if (dev.getHistoryBedList() != null
								&& dev.getHistoryBedList().size() > 0) {
							// 关联信息保存
							for (HistoryBedEntity historyBed : dev
									.getHistoryBedList()) {
								dbManager.saveNoTransaction(historyBed);
							}
						}
						...
						if (dev.getHistoryLocationMoveList() != null
							...
							for (HistoryLocationMoveEntity historyLocationMove : dev
									.getHistoryLocationMoveList()) {
								dbManager
										.saveNoTransaction(historyLocationMove);
							}
						}
						if (dev.getHistoryLocationPosList() != null
								&& dev.getHistoryLocationPosList().size() > 0) {
